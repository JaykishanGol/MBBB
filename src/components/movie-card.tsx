...
export const MovieCard = memo(function MovieCard({ movie }: { movie: Movie }) {
  ...
  return (
    <Link href={`/${movie.media_type}/${movie.id}`} className="block h-full w-full group/card">
      <Card className="group-hover/card:border-primary group-hover/card:shadow-xl group-hover/card:-translate-y-1 group-focus-visible/card:ring-2 group-focus-visible/card:ring-ring group-focus-visible/card:ring-offset-2 relative h-full w-full overflow-hidden rounded-lg border-2 border-transparent transition-all duration-300 ease-out aspect-[2/3] gpu-accelerate">
        <div className="overflow-hidden h-full image-container">
          {movie.poster_path ? (
            <Image
              ...
              fill
              className={cn(
                "h-full w-full object-cover transition-all duration-500 ease-out group-hover/card:scale-105",
